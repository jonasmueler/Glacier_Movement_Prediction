...
datasetTrain = datasetClasses.glaciers(os.path.join(pathOrigin, "datasets", "aletsch"), "train")
dataTrain = DataLoader(datasetTrain, params["batchSize"], shuffle = True)

datasetVal = datasetClasses.glaciers(os.path.join(pathOrigin, "datasets", "aletsch"), "val")
...
functions.trainLoopUnet(dataTrain, dataVal,  model, loss, False, "LSTMAttentionSmallMixedData", params, True, device)
