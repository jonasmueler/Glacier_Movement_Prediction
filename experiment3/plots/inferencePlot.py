...
# create model
#model = visionTransformer.visionFuturePrediction(4,50,5, 2, 5, 2500, 2500, 0.3, 0.3, num_classes = 2500).to(device)
#model = unet_model.UNet(1,1).to(device)
#model = LSTM.LSTM(3,3, 2500, 2500, 0.1, device).to(device)
#model = transformerBase.Transformer(2500, 2, 4, device, predictionInterval=4).to(device)
...
# load weights
model = plottingFunctions.loadCheckpoint(model, None, os.path.join(pathOrigin, "models", "LSTMAttentionSmallMixedData"))
...
with torch.no_grad():
    for i in range(len(sequences)):
        plottingFunctions.inferenceScenes(model, sequences[i], 50, 50, (1, 800, 800),
                                      "parvati", str(i), "LSTMAttentionSmallMixedData", device,
