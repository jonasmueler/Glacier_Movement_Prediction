...
class AE_Transformer(nn.Module):
    ...
    def encoder(self, x, encDate, targets):
        ...
        for i in range(len(x)):
            ...
            if targets == False:
                ...
                if i == 3: # just take last patch in input for conditioning in latent space
                    helper.append(s)
                s, indices = self.maxPool(s)
                s = self.relu(s)
                if i == 3:
    ...
    def forward(self, d, training):
        ...
        if training:
            # decoder
            s = self.decoder(l[0], skipConnections, poolingInd, res[3], 1)  # output encoder: [result, skipConnections, poolingIndices, meanImage]
            ...
            # decoder
            s = self.decoder(l[0], skipConnections, poolingInd, res[3], 1)  # output encoder: [result, skipConnections, poolingIndices]
            s = s.unsqueeze(dim=1) # for loss
